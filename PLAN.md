...
Building it changed the design for the better. `Table.rows` and `Chart.data` are `Record<string, unknown>`, which is an open object no strict schema can close — and they are also precisely what a model must not invent. **They are removed from the schema entirely.** The model composes the table and cites a tool call; the hub fills the rows in from that call's result. A fabricated row is not rejected after the fact, it was never expressible. `StatTile.value` is the case that still needs checking, because it is a string the model composes: citing a real call proves it looked something up and nothing more, so the value must actually appear in that call's result or the tile is refused. That is the difference between a citation and grounding.
...
12. `packages/public-api` — brokered façade over audience-external declarations, versioned independently.
