...
**Grounding is enforceable, not a prompt plea.** Agent-composed specs must carry `source: {toolCallId}` on every data-bearing node — `StatTile.value`, `Table.rows`, `Chart.series`. The hub rejects ungrounded numbers. The model composes layout; only tools supply facts. The agent emits through a `render_screen` tool whose `input_schema` *is* the catalog schema with `strict: true`, so the API constrains output before our validator runs — two layers, neither a prompt.
