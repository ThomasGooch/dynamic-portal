...
**Principle: the hub authenticates; satellites authorize.** The hub establishes the principal (user, tenant, audience, scopes, org roles) and propagates it to satellites. Authentication is **Keycloak OIDC**: the user logs in at Keycloak, the hub exchanges the code and maps the verified claims — org roles from the access token, tenant from a user attribute — into the same `Principal` every call already takes (`principalFromClaims` in the hub, unit-tested against a real token). On the wire to satellites the hub currently signs that `Principal` (HMAC, shared with the Python and C# satellites); replacing that leg with **RFC 8693 token exchange** verified against the issuer's JWKS is the next step, and needs no call-site change because the `Principal` shape does not move. **Every satellite independently enforces tenant scoping — and roles — on every call.** The hub must not become the policy engine.
...
**Org roles are a third axis, and the satellites declare it.** Alongside audience (who a thing is exposed to) and scopes (the capability it demands), a screen or action declares which org **roles** — `leadership`, `engineering`, `finance`, `platform` — may reach it. This is the satellite *informing* the hub who can see and do what, while the hub still owns the decision and the platform registry can narrow it further but never widen it (the same intersection `entitle()` applies to audience; `combine()` treats a layer that declares no roles as no-opinion, so it drops out rather than collapsing the set). Two rules set roles apart, both deliberate. They are **any-of**: a person holds one org role, so holding one of the declared roles suffices — the opposite of scopes, where every one is required. And they are **opt-in**: a screen that declares no roles is not role-gated, so everything predating roles keeps working and `undefined` (un-gated) stays distinct from `[]` (nobody) — for roles, "forgot to declare" is not "locked down", the inverse of audience's fail-closed default. Roles gate **internal** principals only: an external partner carries no org role, and their access stays governed by audience, scopes, and the public projection, so role-gating an externally-facing satellite never locks partners out. As with everything else here, each satellite re-checks roles itself, and the roles claim travels on the same strict cross-language `Principal` — which forces a rollout order (satellites accept the claim first, then the hub begins sending it).
...
**Audit is a first-class schema, not a log format.** Append-only, records: principal, tenant, audience, satellite, screen/tool, parameter digest, outcome, latency, and — for agent actions — the complete tool-call chain. This is the artifact a regulator or client security review asks for. Design it in M1; retrofitting audit into a system that already handles regulated data is a re-certification, not a patch.

**The digest is keyed per tenant, and the key is mandatory.** `PORTAL_AUDIT_KEY` and `PORTAL_AUDIT_LOG` have no defaults, and without them the hub's server fails to prepare in Next's `instrumentation` hook — every request including `/` answers 500, so the container never passes its healthcheck. The process does stay up, which is why this says what happens rather than "refuses to start": a default key produces a log that looks keyed and is not, a default path one nobody knows to collect. Both look like success, which is what makes them worse than refusing. Keys derive per tenant from one configured secret, so one tenant's log cannot be correlated against another's — and handing a tenant their own log gives them nothing about anyone else's. That closes the gap the schema shipped with: an unkeyed SHA-256 over a low-entropy parameter is recoverable by anyone holding the log and a candidate list, which is most people who would ever read it.
...
11. **Regulated-data audit** — from the audit log alone, answer "which records did the agent read, for whom, when." *(Asserted end to end: a screen read, an action, per-tenant digests that differ for the same request, and no scope or parameter anywhere in the file.)*
...
- **A full audit disk is an outage.** Audit writes fail closed, so a request that cannot be recorded does not proceed. That is the only posture consistent with calling the audit mandatory, and the cost is that the log's storage is now on the critical path. An operator should treat it as such: separate volume, monitored, rotated.
...
- **The agent conversation is signed, because the hub is stateless between turns and it comes back as client input.** Everything grounding believes about what a tool returned is rebuilt from the `tool_result` blocks in that history, so an unsigned one let a client fabricate a result and receive a screen of invented figures wearing a provenance citation. Every turn the hub issues carries an HMAC over the conversation *and the subject it was issued to*, keyed per tenant and separated by purpose from the audit key; the next turn is refused unless it verifies. The subject is in the signed payload rather than only the key because two users in one tenant share a key but not their entitlements, and a replayed history is evidence grounding re-renders without re-fetching. The request separates `history` (signed, hub-issued) from `ask` (what the user just typed), because a signature can only cover what the hub actually sent. The body is size-capped before it is parsed, at the same 256 KB every other route uses — the signature can only be checked after the parse, so an uncapped route paid the cost of an arbitrary body before it had any reason to trust it. Because the history accumulates while the cap does not, the hub trims the conversation on the way *out* — dropping the oldest whole turns, never cutting inside one, and signing what actually left — so it never issues a conversation it will refuse next turn. A cap on something that only grows is a cliff unless something keeps the growth inside it. What remains uncapped is a single turn larger than the budget on its own: trimming within a turn would mean editing a `tool_result` that grounding cites, and a citation resolving to a doctored result is worse than a conversation that ends.

  **What this deliberately does not defend against:** a user approving a write without reading the confirmation card. They are authenticated, authorized, and could call the action endpoint directly with their own credentials. The confirmation gate protects against the *model* acting unbidden; the signature is what stops the model's output being edited on the way through a browser.
...
- **The vendor call is now exercised, and it found something no scripted test could.** The API accepts the 34-variant `render_screen` schema, and the model composes a valid spec — but the first live run burned every turn it had, because grounding demands a `toolCallId` and nothing ever told the model what one *was*. It cited the tool's name, twice. Every scripted test passed correct ids because the author wrote them. The id now travels in the read's own result, a failed citation names the ids that exist, and composition succeeds on the first attempt.

  The live path is covered by e2e tests that skip unless the running stack has a key, so CI stays green without one and a developer with the Azure AI Foundry vars (`AZURE_OPENAI_API_KEY`, `PORTAL_AZURE_ENDPOINT`, `PORTAL_AZURE_DEPLOYMENT`) in `.env` exercises the one thing a script cannot: a model deciding what to do.
